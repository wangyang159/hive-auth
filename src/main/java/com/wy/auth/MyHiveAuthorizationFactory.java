...
public class MyHiveAuthorizationFactory implements HiveAuthorizerFactory {
    ...
    public HiveAuthorizer createHiveAuthorizer(HiveMetastoreClientFactory hiveMetastoreClientFactory, HiveConf hiveConf, HiveAuthenticationProvider hiveAuthenticationProvider, HiveAuthzSessionContext hiveAuthzSessionContext) throws HiveAuthzPluginException {
        ...
        String driver = hiveConf.get("hive.auth.database.driver");
        String timeout = hiveConf.get("hive.auth.database.timeout");
        ...
        //链接超时
        BigInteger timeout_bi = new BigInteger(timeout);
        ...
        //初始化连接池配置
        MysqlUtil.init(url,driver,timeout_bi.longValue(),username,password,hp_maxsize_bi.intValue(),hp_minidle_bi.intValue(),hp_id_timeout_bi.longValue(),hp_lefttime_bi.longValue());
